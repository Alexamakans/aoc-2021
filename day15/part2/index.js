...
function exec(path) {
	...
	for (let right = 0; right < 5; right++) {
		for (let down = 0; down < 5; down++) {
			const inc = right + down;
			const newTile = [];
			for (let y = 0; y < weights.length; y++) {
				newTile[y] = [];
				for (let x = 0; x < weights[y].length; x++) {
					newTile[y][x] = weights[y][x] + inc;
					if (newTile[y][x] > 9) {
						newTile[y][x] -= 9;
					}
				}
			}
			expanded[right][down] = newTile;
		}
	}

	function getWeight(x, y) {
		const bigX = Math.floor(x / weights[0].length);
		const bigY = Math.floor(y / weights.length);
		const smallX = x % weights[0].length;
		const smallY = y % weights.length;

		return expanded[bigX][bigY][smallY][smallX];
	}

	const newWeights = [];
	for (let y = 0; y < weights.length * 5; y++) {
		newWeights[y] = [];
		for (let x = 0; x < weights[0].length * 5; x++) {
			newWeights[y][x] = getWeight(x, y);
		}
	}

	const start = [0, 0];
	const goal = [newWeights[0].length - 1, newWeights.length - 1];

	return AStar(newWeights, start, goal);
}

const testResult = exec(testPath);
if (testResult != 315) {
	throw `Test 2 failed: ${testResult}, should be 315`;
}
